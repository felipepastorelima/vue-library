...
import bookListExporterFields from '@/modules/book/book-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!bookListExporterFields || !bookListExporterFields.length) {
          throw new Error('bookListExporterFields is required');
        }
